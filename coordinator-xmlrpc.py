...
def last_reading_by_node():
    ...
    # fixme: seq scan
    c.execute("select outdoor_env.address, db_time::varchar, bmp085_temp_decic, bmp085_press_pa, batt_mv, panel_mv from outdoor_env, (select address, max(db_time) from outdoor_env group by address) a where outdoor_env.address = a.address and outdoor_env.db_time = a.max;")
...
def last_hour_avg_by_node():
    c = db_conn.cursor()
    c.execute("select address, min(db_time)::varchar as min_db_time, max(db_time)::varchar as max_db_time, count(*) as n, avg(bmp085_temp_decic)::double precision as avg_bmp085_temp_decic, avg(bmp085_press_pa)::double precision as avg_press_pa from outdoor_env where db_time > now() - '1 hour'::interval group by address;")
